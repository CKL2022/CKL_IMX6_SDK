...
dd if=../image/u-boot.imx of=$node bs=1024 seek=1 conv=fsync
sync
...
echo "正在解压overlay到${node}2 ，请稍候..."

rm -rf mount_point0/lib/modules/*

tar jxf ../image/overlay.tar.bz2 -C mount_point0/
sync
echo "解压overlay到${node}2完成！"
...
do
	...
	cp -a ../image/*.imx ../image/zImage ../image/*.dtb ../image/*.tar.bz2   mount_point0/mk_emmc/image/
	sync
