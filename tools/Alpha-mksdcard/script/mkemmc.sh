...
if [ ! -e $files/zImage ]; then
  echo "错误: ../image/下找不到zImage"
  exit 1
fi

if [ ! -f $files/overlay.tar.bz2 ]; then
  echo "错误: $files/下找不到overlay"
  exit 1
fi
...
uboot=u-boot.imx
dd if=$files/$uboot of=${node}boot0 bs=1024 seek=1 conv=fsync
dd if=$files/$uboot of=${node}boot0 bs=1024 seek=500 conv=fsync
...
echo "正在解压到${node}p2/，请稍候..."
overlay=$files/overlay.tar.bz2
tar jxf $overlay -C ./mount1/
chown -R 0.0 mount1/lib/*
sync
echo "解压overlay到${node}p2/完成！"
